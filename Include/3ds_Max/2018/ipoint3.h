...
public:
	...
	IPoint3(const int ai[3]) { x = ai[0]; y = ai[1]; z = ai[2]; }

	// Access operators
	/*! \remarks Allows access to x, y and z using the [ ] operator.
	\return  An index of 0 will return x, 1 will return y, 2 will return z. */
	int& operator[](int i) { DbgAssert((i >= 0) && (i <= 2)); return (&x)[i]; }     
	/*! \remarks Allows access to x, y and z using the [ ] operator.
	\return  An index of 0 will return x, 1 will return y, 2 will return z. */
	const int& operator[](int i) const { DbgAssert((i >= 0) && (i <= 2)); return (&x)[i]; }
...
inline IPoint3& IPoint3::operator*=(const IPoint3& rhs) 
{
    x *= rhs.x;	
    y *= rhs.y;	
    z *= rhs.z;
