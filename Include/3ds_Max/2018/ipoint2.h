...
public:
	...
	int& operator[](int i) { DbgAssert((i == 0) || (i == 1)); return (&x)[i]; }     
	/*! \remarks Allows access to x, y using the subscript operator.
	\return  An index of 0 will return x, 1 will return y. */
	const int& operator[](int i) const { DbgAssert((i == 0) || (i == 1)); return (&x)[i]; }
...
inline int Length(const IPoint2& v) {
	double dvx = (double)v.x;
	double dvy = (double)v.y;
	return (int)sqrt(dvx * dvx + dvy * dvy);
	}
...
inline IPoint2& IPoint2::operator*=(const IPoint2& rhs) 
{
    x *= rhs.x;   
    y *= rhs.y;
    return *this;
