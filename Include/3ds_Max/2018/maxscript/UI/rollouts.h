...
public:
	...
	bool			inDelete;
				...
				RolloutFloater();
				RolloutFloater(const MCHAR* title, int left, int top, int width, int height, HWND parentHWnd);
	 			~RolloutFloater();
#	define		is_rolloutfloater(v) ((DbgVerify(!is_sourcepositionwrapper(v)), (v))->tag == class_tag(RolloutFloater))

	void		Init(const MCHAR* ititle, int iLeft, int iTop, int iWidth, int iHeight, HWND parentHWnd);
