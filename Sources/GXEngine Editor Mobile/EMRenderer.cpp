...
GXVoid EMRenderer::StartCommonPass ()
{
	...
	if ( status != GL_FRAMEBUFFER_COMPLETE )
		GXLogW ( L"EMRenderer::StartCommonPass::Error - ﾗ- 淲 ・・FBO (ⅹ鞦・ 0x%08x)\n" );
...
GXVoid EMRenderer::StartLightPass ()
{
	...
	GLenum status = glCheckFramebufferStatus ( GL_FRAMEBUFFER );
	if ( status != GL_FRAMEBUFFER_COMPLETE )
		GXLogW ( L"EMRenderer::StartLightPass::Error - ﾗ- 淲 ・・FBO (ⅹ鞦・ 0x%08x)\n" );
...
GXVoid EMRenderer::StartHudColorPass ()
{
	...
	GLenum status = glCheckFramebufferStatus ( GL_FRAMEBUFFER );
	if ( status != GL_FRAMEBUFFER_COMPLETE )
		GXLogW ( L"EMRenderer::StartHudDepthDependentPass::Error - ﾗ- 淲 ・・FBO (ⅹ鞦・ 0x%08x)\n" );
...
GXVoid EMRenderer::CreateFBO ()
{
	...
	GLenum status = glCheckFramebufferStatus ( GL_FRAMEBUFFER );
	if ( status != GL_FRAMEBUFFER_COMPLETE )
		GXLogW ( L"EMRenderer::CreateFBO::Error - ﾗ- 淲 ・・FBO (ⅹ鞦・ 0x%08X)\n", (GXUInt)status );
