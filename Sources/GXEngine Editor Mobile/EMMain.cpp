...
GXVoid GXCALL EMOnObject ( GXUPointer object )
{
	...
	GXLogW ( L"EMOnObject::Info - ÎáúåêE%s (àäðåE0x%016X)\n", actor->GetName (), object );
}
...
GXVoid GXCALL EMOnOpenFile ( const GXWChar* filePath )
{
	GXLogW ( L"EMOnOpenFile::Info - ÔàéE %s\n", filePath );
}
...
GXVoid GXCALL EMOnInitRenderableObjects ()
{
	...
	em_LanguagePopup->AddItem ( locale->GetString ( L"Language->Ðóññêèé" ), nullptr );
	...
	em_EditBox->SetText ( L"Ïðèâå, Ñèñüêè" );
