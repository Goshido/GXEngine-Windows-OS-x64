...
GXBool GXCALL GXLoadFile ( const GXWChar* fileName, GXVoid** buffer, GXUInt &size, GXBool notsilent )
{
	...
	if ( input == 0 )
	{
		if ( notsilent )
		{
			GXDebugBox ( L"GXLoadFile::Error - ﾍ・・胚 ⅳ・靑・鴉" );
			GXLogW ( L"GXLoadFile::Error - ﾍ・・胚 ⅳ・靑・鴉 %s\n", fileName );
		}
	...
	if ( fileSize == 0 )
	{
		GXDebugBox ( L"GXLoadFile::Error - ﾔ琺・・" );
		GXLogW ( L"GXLoadFile::Error - ﾔ琺・%s ・\n", fileName );
	...
	*buffer = (GXVoid*)malloc ( fileSize );
	...
	if ( readed != fileSize )
	{
		GXDebugBox ( L"GXLoadFile::Error - ﾍ・・胚 ・ⅸ頸瑣・鴉" );
		GXLogW ( L"GXLoadFile::Error - ﾍ・・胚 ・ⅸ頸瑣・鴉 %s\n", fileName );
...
GXBool GXCALL GXWriteToFile ( const GXWChar* fileName, const GXVoid* buffer, GXUInt size )
{
	...
	if ( !input )
	{
		GXDebugBox ( L"GXWriteToFile::Error - ﾍ・・胚 鈕瑣・鴉" );
		GXLogW ( L"GXWriteToFile::Error - ﾍ・・胚 鈕瑣・鴉 %s\n", fileName );
